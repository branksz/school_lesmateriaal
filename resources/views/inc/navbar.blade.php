    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav flex-lg-row ml-lg-auto d-lg-none d-lg-flex mr-3">
                @foreach ($nav as $item)
                    @if($item['title'] == 'Inloggen' && (Auth::user() || Auth::guard('admin')->check()))
                        <li class="nav-item">
                            <li class="nav-item ml-2 mr-2 btn-secondary">
                                <a class="nav-link text-white" href="{{ Auth::guard('admin')->check() ? url('/admin/dashboard') : url('/dashboard') }}">Dashboard</a>
                            </li>
